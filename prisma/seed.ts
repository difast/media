...
// Section definitions mirrored from src/lib/site-config (kept inline so the
// seed has no app-runtime dependency).
const SECTIONS = [
...
// A curated set of stock photo URLs (Unsplash) for premium visuals.
const PHOTOS = [
...
async function main() {
  ...
  for (let i = 0; i < SECTIONS.length; i++) {
    ...
    const c = await prisma.category.upsert({
      where: { slug: s.slug },
      update: { title: s.ru, titleEn: s.en, kind: s.kind as any, order: i },
      create: { slug: s.slug, title: s.ru, titleEn: s.en, kind: s.kind as any, order: i },
    });
    categories.push(c);
  }
  ...
  const admin = await prisma.user.upsert({
    ...
    create: {
      email: adminEmail,
      name: "Главная редакция",
      slug: "editorial",
      position: "Главный редактор",
      role: "EDITOR",
      passwordHash: await bcrypt.hash(adminPass, 10),
      bio: "Редакция Pyatakov Media.",
    },
  });
  ...
  for (let i = 0; i < TITLES.length; i++) {
    ...
    const publishedAt = new Date(now - i * 7 * 3600 * 1000);
    ...
    const article = await prisma.article.upsert({
      ...
      create: {
        ...
        coverImage: PHOTOS[i % PHOTOS.length],
        coverCaption: "Фото: Pyatakov Media",
    ...
    // First revision (история изменений)
  ...
  for (let i = 0; i < interviewData.length; i++) {
    ...
    await prisma.article.create({
      data: {
        ...
        categoryId: interviewCat.id,
        interview: {
          create: {
            guestName: d.guest,
            guestTitle: d.title,
            guestPhoto: PORTRAITS[i % PORTRAITS.length],
            ...
            timecodes: [
              { time: "00:00", label: "Вступление" },
              { time: "05:30", label: "О рынке" },
              { time: "18:40", label: "Прогнозы" },
            ],
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await prisma.article.create({
      data: {
        slug,
        status: "PUBLISHED",
        title: `Выпуск №${i + 1}: разговоры о деньгах и технологиях`,
        subtitle: "Подкаст Pyatakov Media",
        excerpt: "Еженедельный подкаст о бизнесе, инвестициях и технологиях.",
        body: BODY,
        coverImage: PHOTOS[(i + 5) % PHOTOS.length],
        readingMinutes: 1,
        publishedAt: new Date(now - (i + 1) * 3 * 24 * 3600 * 1000),
        authorId: admin.id,
        ...
        podcast: {
          create: {
            episodeNumber: i + 1,
            duration: `${40 + i}:1${i}`,
            seriesName: "Деньги и технологии",
            links: [
  ...
  await prisma.ad.upsert({
    where: { id: "seed-ad-sidebar" },
    update: {},
    create: {
      id: "seed-ad-sidebar",
      title: "Реклама в Pyatakov Media",
      slot: "SIDEBAR",
      targetUrl: "/advertising",
      html: "<div>Ваша реклама здесь</div>",
      isActive: true,
    },
  });
  ...
  console.log(`✅ Done. Articles: ${created}, authors: ${authors.length}, categories: ${categories.length}.`);
